...
class EmailService {
  ...
  async sendEmail(options: SendEmailOptions): Promise<boolean> {
    try {
      // TODO: Log do email no banco (quando EmailLog table estiver criada)
      ...
      //     usuario_email: options.destinatario,
      ...
      // TODO: Atualizar status para enviado (quando EmailLog table estiver criada)
      ...
      // TODO: Atualizar status para erro (quando EmailLog table estiver criada)
      // try {
      ...
      //   });
      // } catch (logError) {
      //   console.error('Erro ao atualizar log de email:', logError);
      // }
  ...
  async sendPasswordResetEmail(nome: string, email: string, resetToken: string): Promise<boolean> {
    const html = this.generatePasswordResetEmail(nome, resetToken);

    return await this.sendEmail({
      destinatario: email,
      assunto: '🔐 Redefinir Senha - Horizonte do Saber',
      corpo: html,
      tipo: 'reset_senha',
      usuarioNome: nome,
    });
